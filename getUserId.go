...
func ggetUserIdsAll(execConfig *ExecutorConfig, threads *int, okClient *OktaClient) {
	...
	for _, f := range *execConfig.FilesToProcess {
		...
		for {
			...
			go func(record []string, m int) {
				defer wg.Done()
				ch <- record
				getUserId(record[0], okClient, execConfig)
...
func getUserId(User string, okClient *OktaClient, execConfig *ExecutorConfig) (jobStatus bool) {
